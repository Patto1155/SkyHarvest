...
|---|---|---|---|---|
| G1 | **Camera zoom absent**; default `orthographicSize=4` frames whole island, avatar tiny | spec §7 "Fixed camera — zoom in/out only", "good camera zoom range" | `Bootstrap.cs:75`, `CameraFollow.cs` follow-only | This session (task 1) |
| G2 | **Staged building missing** — `TryPlace` consumes materials and spawns finished structure in one click | spec §2 "place a translucent ghost → deliver materials → construction completes"; §9 "Deliver materials to complete construction" | `BuildModeController.TryPlace` | This session (task 2) |
| G3 | **Build mode broken in New Game**: `BuildModeController.SetIsland` only called in `StartFromSave` (`Bootstrap.cs:306`); `_island==null` → `CanPlaceAt` always false | plan Task 15/21 wiring | grep: no `bmc.SetIsland` in `StartNewGame` | Fix this session |
| G4 | **`SetPlayer` never called** → `_player==null` → build cost check + consumption silently skipped (free builds, on the save path where G3 doesn't mask it) | plan Task 15 | grep: zero callers of `BuildModeController.SetPlayer` | Fix this session |
| G5 | **Inventory has no keybind** — `InventoryUI.Toggle()` has zero callers; tooltip text claims "Tab opens your pack" (false) | plan Task 5: `OpenInventory` bound to Tab | `InventoryUI.cs:30`, `ContextualTooltipUI.cs:42-43` | Fix this session (bind Tab; see keybind table) |
| G6 | **Hotbar mismatch**: HUD renders 6 slots, only keys 1–4 wired to 4 tools; slots 5–6 dead | spec §9 hotbar; NEXT_SESSION said "1–6" | `Bootstrap.cs:104-114` (6 slots), `ToolSystem.cs` (4 tools) | Verify pass; either wire 5–6 (seeds?) or shrink HUD to 4 — defer decision, note in verify table |
| G7 | **Esc double-fire risk**: `Bootstrap.Update` toggles pause on Esc unconditionally once game started; StorageUI/BuildMenu/BuildMode also consume Esc the same frame → closing a panel likely also opens pause | spec: Esc = cancel/close | `Bootstrap.cs:453`, `StorageUI.cs:51`, `BuildMenuUI.cs:53`, `BuildModeController.cs:59` | Verify + fix this session |
...
| 1–6 | Hotbar slots (HUD shows 6) | 1–4 only → Hoe/WateringCan/Sickle/Hammer (`ToolSystem.cs:30` + duplicate handler `PlayerController.cs:133`) | ⚠ G6 |
